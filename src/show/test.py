...
def visualize_cad_json(json_path):
    ...
    # 2D草图可视化 - 只可视化第一个组件的草图
    ax1 = fig.add_subplot(121)
    
    # 获取所有组件的数据
    components = data["assembly"]["components"]
    first_component_key = list(components.keys())[0]
    first_component = components[first_component_key]
    
    if first_component["type"] == "extruded_sketch":
        sketch = first_component["sketch"]
        patches = []
        ...
        if component["type"] == "extruded_sketch":
            ...
            for contour in sketch["contours"]:
                if contour["type"] == "rectangle":
                    ...
                    # 修复: 创建拉伸路径 - 确保x, y, z维度匹配
                    num_z_points = 10
                    z_values = np.linspace(0, z_max, num_z_points)
                    
                    for i in range(len(points)-1):
                        # 在两点之间插值生成与z相同数量的点
                        x_values = np.linspace(points[i][0], points[i+1][0], num_z_points)
                        y_values = np.linspace(points[i][1], points[i+1][1], num_z_points)
                        
                        # 现在x_values, y_values和z_values具有相同的形状
                        ax2.plot(x_values, y_values, z_values, color='green', alpha=0.7)
...
visualize_cad_json("../../data/example/cylinder.json")
visualize_cad_json("../../data/example/L_bracket.json")
visualize_cad_json("../../data/example/rec_board.json")